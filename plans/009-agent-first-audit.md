...
| --- | --- | --- | --- |
| SEC01 | P1 / package audit | `examples/smoke/pnpm-workspace.yaml` pins `js-yaml` 4.3.1; `@cloudflare/vite-plugin → miniflare → sharp` is also affected. High advisories [GHSA-2883-xcg3-v3hh](https://github.com/advisories/GHSA-2883-xcg3-v3hh) and [GHSA-rgj7-g3m4-5g8c](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c). Development-dependency exposure, not automatically a production exploit. | **Open.** Patch through compatible parent updates or a justified temporary override, then rebuild. Track all six moderate findings too. |
...
| AR05 | P1 / static | Root and smoke ignores miss common dotenv variants. Astro's own `.gitignore` only ignores `dist-preview/`, relying on its parent. | **Open.** Self-contained secret/artifact exclusions; keep placeholder examples trackable. |
