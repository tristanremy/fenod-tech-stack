...
| AR15 | P1 / observed generator behaviour | `pnpm generate-routes` (`tsr generate`, `@tanstack/router-cli` 1.132.0) is **not deterministic on consecutive runs**. It alternates between two outputs: every second run drops the trailing `declare module '@tanstack/react-start'` block (the `interface Register { ssr: true; router: Awaited<ReturnType<typeof getRouter>> }` augmentation) that the committed file contains. Two consecutive runs produced 77 lines (`5cf738…`) then 86 lines (`19c325…`), alternating. Consequence: re-running the generator can silently remove SSR/Start type registration, and a “generated files are fresh” check cannot be trusted for this file. | **Open, recorded.** The committed tree is pinned to the variant that keeps the block; do not re-run `pnpm generate-routes` without inspecting the diff. Re-verify against the installed router-cli version and report upstream before building any freshness gate on this file. |
