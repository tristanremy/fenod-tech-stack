...
const record = (event) => appendFileSync(process.env.FIXTURE_TRACE, `${event}\n`);
...
globalThis.fetch = async (request) => {
  ...
  assert.equal(request.headers.get("authorization"), "Bearer dp.st.dev.synthetic-offline-fixture");
  ...
  return Response.json({ secrets: {
    ...
    BETTER_AUTH_SECRET: { computed: "test-only-offline-canary-012345678901234567890123" },
  } });
};
