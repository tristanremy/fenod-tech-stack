...
try {
  ...
  const install = spawnSync(
    ...
    [
      ...
      "--save-exact",
      `varlock@${VARLOCK}`,
      `@varlock/doppler-plugin@${DOPPLER_PLUGIN}`,
    ],
    {
      cwd: directory,
      env,
  ...
  const pkg = JSON.parse(
    readFileSync(join(directory, "node_modules/varlock/package.json"), "utf8"),
  );
  const cli = join(
    directory,
    "node_modules/varlock",
    typeof pkg.bin === "string" ? pkg.bin : pkg.bin.varlock,
  );
  const preload = fileURLToPath(
    new URL("./fixtures/doppler-transport.mjs", import.meta.url),
  );
  ...
  const load = (mode) => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...env,
          DOPPLER_TOKEN: "dp.st.dev.synthetic-offline-fixture",
          FIXTURE_MODE: mode,
          FIXTURE_TRACE: trace,
        },
      },
    ...
    assert.equal(
      result.error,
      undefined,
      `${mode}: process failed/timed out, not a valid negative result`,
    );
    const events = readFileSync(trace, "utf8").trim().split("\n");
    assert.ok(
      events.includes(mode),
      `${mode}: must reach mocked Doppler transport`,
    );
    assert.ok(
      events.every((event) => event === mode || event === "timeout-aborted"),
    ...
    if (mode === "success") {
      ...
      assert.equal(
        report.config.BETTER_AUTH_URL.value,
        "http://localhost:3000",
      );
      ...
      for (const key of ["APP_ENV", "BETTER_AUTH_URL", "BETTER_AUTH_SECRET"]) {
        assert.ok(
          errors?.[key],
          `${mode}: requires a resolution error on ${key}`,
        );
        assert.ok(
          !report.config?.[key]?.value,
          `${mode}: must not return a previous value for ${key}`,
        );
      }
      const reason = {
        ...
        "not-found": /Project or config not found/,
        "rate-limited": /429/,
        unavailable: /503/,
      }[mode];
      assert.match(
        ...
        reason,
        `${mode}: wrong failure reason`,
      );
      if (mode === "timeout") assert.ok(events.includes("timeout-aborted"));
      // Transient statuses must be retried, not fail on the first response.
      if (mode === "rate-limited") assert.ok(events.length > 1, "429 was not retried");
    }
  ...
  process.stdout.write(
    "PASS fresh-load failure and recovery with cache disabled; running-process memory is not tested.\n",
  );
} finally {
