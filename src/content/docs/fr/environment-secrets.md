---
title: "Environnements et secrets"
verified: 2026-06
---
...
### À garder comme configuration non secrète
...
## Options de déploiement Cloudflare

### Option A: Infisical Cloudflare Workers Sync

Utilisez this when you want Cloudflare to hold the runtime secrets before deploy.
...
Utilisez this when Alchemy needs the secret value while creating/updating resources:
...
Utilisez this for GitHub Actions:

1. Create an Infisical machine identity that trusts GitHub OIDC for the repo/workflow.
2. Utilisez Infisical's GitHub action to fetch secrets at job runtime.
...
Keep the plain scripts useful for CI jobs that already fetched secrets. Utilisez the `:*:secrets` scripts locally.

## Pattern runtime Cloudflare Worker
...
Utilisez `process.env` only in Node-side tooling such as Alchemy, Drizzle config, scripts, and CI.

## CAPM-Inspired Deployment Hardening

Bring these patterns into new Fenod projects:

- Utilisez a single `STAGE` variable (`dev`, `staging`, `prod`) and derive resource names from it.
- Keep domain maps in `alchemy.run.ts` so staging/prod hostnames are explicit and reviewable.
- Utilisez isolated D1/R2/KV/Queue resources per stage.
